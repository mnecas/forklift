...
func (r *Validator) isIpInSubnet(ip string, subnet string) (ok bool, err error) {
	_, ipNet, err := net.ParseCIDR(subnet)
	if err != nil {
		return false, fmt.Errorf("error parsing CIDR: %s", subnet)
	}
...
func (r *Validator) UdnStaticIPs(vmRef ref.Ref, client client.Client) (ok bool, err error) {
	...
	for _, guestNetwork := range vm.GuestNetworks {
		if guestNetwork.Network == sourceNetwork.Name {
			...
			if err != nil {
				return false, fmt.Errorf("error parsing CIDR: %s", udnSubnet)
			}
