...
TOEHOLD_IMPORTER_IMAGE ?= $(REGISTRY)/$(REGISTRY_ORG)/toehold-importer:$(REGISTRY_TAG)
...
build-operator-bundle-image: check_container_runtime
	$(CONTAINER_CMD) build $(PLATFORM_FLAG) $(BUILD_LABEL_ARGS) \
		...
		--build-arg TOEHOLD_IMPORTER_IMAGE=$(TOEHOLD_IMPORTER_IMAGE)$(PLATFORM_SUFFIX) \
...
build-toehold-importer-image: check_container_runtime
	$(CONTAINER_CMD) build $(PLATFORM_FLAG) $(BUILD_LABEL_ARGS) -t $(TOEHOLD_IMPORTER_IMAGE)$(PLATFORM_SUFFIX) -f build/toehold/Containerfile .

push-toehold-importer-image: build-toehold-importer-image
	$(CONTAINER_CMD) push $(TOEHOLD_IMPORTER_IMAGE)$(PLATFORM_SUFFIX)
