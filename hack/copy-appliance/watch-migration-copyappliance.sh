...
ca=\$(oc get copyappliance -n '${namespace}' -l migration=\$(oc get migration '${migration}' -n '${namespace}' -o jsonpath='{.metadata.uid}') -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || true)
if [[ -n \"\${ca}\" ]]; then
